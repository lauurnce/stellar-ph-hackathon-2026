...
function StatusPill({ status }) {
  const map = {
    ...
    "Delivered · Review": {
      ...
      border: "rgba(46,175,125,.35)",
      text: "#FF8C5A",
      dot: "#2EAF7D",
    },
    Completed: {
      bg: "rgba(68,147,66,.14)",
      border: "rgba(68,147,66,.35)",
      text: "#7ECFC6",
      dot: "#449342",
    },
    Disputed: {
...
function Btn({
  ...
  const variants = {
    coral: {
      background: h
        ? `linear-gradient(135deg,#3FD0C9,${C.coralDark})`
        : `linear-gradient(135deg,${C.coral},${C.coralDark})`,
      color: "#fff",
      border: "none",
      boxShadow: h
        ? "0 10px 36px rgba(46,175,125,.5),0 0 0 1px rgba(46,175,125,.35)"
    ...
    blue: {
      background: h
        ? "linear-gradient(135deg,#3FD0C9,#3FD0C9)"
        : "linear-gradient(135deg,#3FD0C9,#2AADA7)",
      color: "#fff",
      border: "none",
      boxShadow: h
        ? "0 8px 28px rgba(63,208,201,.45)"
        : "0 4px 16px rgba(63,208,201,.28)",
    },
  };
  return (
    <button
...
function toDisplayEscrow(escrow) {
  ...
  return {
    ...
    action: {
      label: status === "Delivered · Review" ? "Review" : "View Details",
      variant: status === "Delivered · Review" ? "coral" : "subtle",
    },
  };
}

function EscrowTableBody({
  rows,
  loading,
  error,
  emptyMsg = "No escrows yet.",
}) {
  const cols = [
    "Project",
    "Freelancer",
      ...
      <div style={{ minWidth: 780 }}>
        ...
        {loading ? (
          <TableMessage
            ...
            message="Reading the latest contracts from Supabase."
          />
        ) : error ? (
          <TableMessage
  ...
  return (
    <>
      <div
        style={{
          background:
            "linear-gradient(135deg,rgba(24,24,32,.97),rgba(18,18,26,.97))",
      ...
      >
        <EscrowTableBody
          ...
          loading={loading}
          error={error}
          emptyMsg="No active escrows. Create one to get started."
        />
      </div>
      {done.length > 0 && (
        <div
  ...
  return (
    <div
      style={{
        padding: "34px 24px",
        textAlign: "center",
        borderTop: `1px solid rgba(36,36,48,.7)`,
      }}
    >
      <div
        style={{
          fontSize: 14,
          fontWeight: 800,
          color: tone === "error" ? "#F87171" : C.text,
          marginBottom: 6,
        }}
      >
        {title}
      ...
      {actionLabel && (
        <Btn variant="coral" size="sm" onClick={() => go("/create-escrow")}>
          {actionLabel}
        </Btn>
      )}
    </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <Avatar
          initials={row.freelancer.initials}
          size={32}
          color={row.freelancer.color}
        />
      ...
      {/* Milestone */}
      <div>
        <div style={{ fontSize: 12.5, color: C.textSub, marginBottom: 4 }}>
          {row.milestone}
        </div>
        <div
          style={{
      ...
      <div>
        <Btn
          ...
          size="sm"
          onClick={() => go(row.id ? `/escrow?id=${row.id}` : "/escrow")}
        >
          {row.action.label}
        </Btn>
      </div>
              ...
              >
                <StatCard
                  ...
                  value={
                    loadingEscrows
                      ? "..."
                      : `$${escrows.reduce((sum, e) => sum + parseFloat(e.amount.replace(/[$,]/g, "")), 0).toLocaleString(undefined, { maximumFractionDigits: 0 })}`
                  }
                  sub="Synced from Supabase"
                  color={C.coral}
                  trend={12}
                />
                <StatCard
                  ...
                  value={loadingEscrows ? "..." : escrows.length}
                  sub="Contracts in database"
                  color={C.blue}
                />
                <StatCard
                  icon="✅"
...
function QuickTips() {
  return (
    ...
    >
      <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12.5, color: C.textSub, lineHeight: 1.6 }}>
            Set milestones, lock funds, and get blockchain receipts — in under 2
            minutes.
          </div>
        </div>
      </div>
      ...
      <div
        style={{
          borderTop: `1px solid rgba(46,175,125,.18)`,
          paddingTop: 14,
          display: "flex",
          flexDirection: "column",
          gap: 10,
